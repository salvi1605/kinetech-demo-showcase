...
-- Update validate_and_create_appointment to check practitioner_treatments
CREATE OR REPLACE FUNCTION public.validate_and_create_appointment(
  p_clinic_id uuid, p_practitioner_id uuid, p_patient_id uuid, p_date date,
  p_start_time time without time zone, p_sub_slot integer DEFAULT 1,
  p_treatment_type_key text DEFAULT 'fkt'::text, p_notes text DEFAULT ''::text,
...
BEGIN
  ...
  -- 4b. NEW: Verificar que el profesional puede realizar este tratamiento
  SELECT EXISTS(
    SELECT 1 FROM practitioner_treatments WHERE practitioner_id = p_practitioner_id
  ) INTO v_has_any_pt;

  IF v_has_any_pt THEN
    -- Resolver treatment_type_id primero
  ...
  -- 5. Verificar conflicto de tratamiento exclusivo
  ...
  -- 6. Resolver treatment_type_id (si no se resolvió arriba)
...
-- Update validate_and_update_appointment (the one with p_sub_slot parameter)
CREATE OR REPLACE FUNCTION public.validate_and_update_appointment(
  p_appointment_id uuid, p_practitioner_id uuid DEFAULT NULL::uuid,
  p_date date DEFAULT NULL::date, p_start_time time without time zone DEFAULT NULL::time without time zone,
  p_status text DEFAULT NULL::text, p_treatment_type_key text DEFAULT NULL::text,
  p_notes text DEFAULT NULL::text, p_sub_slot integer DEFAULT NULL::integer
)
RETURNS json
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  -- 3b. NEW: Verificar que el profesional puede realizar este tratamiento
  SELECT EXISTS(
