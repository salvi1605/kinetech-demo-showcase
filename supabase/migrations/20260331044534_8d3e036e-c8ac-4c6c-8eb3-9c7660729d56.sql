-- 1. CHECK constraint on sub_slot
ALTER TABLE appointments
  ADD CONSTRAINT chk_sub_slot_range CHECK (sub_slot >= 1 AND sub_slot <= 10);

-- 2. Recreate validate_and_create_appointment with early sub_slot validation
DROP FUNCTION IF EXISTS public.validate_and_create_appointment(uuid,uuid,uuid,date,time,integer,text,text,text);
...
BEGIN
  -- ── 0. Defensive sub_slot validation ──
  IF p_sub_slot IS NULL OR p_sub_slot < 1 THEN
    p_sub_slot := 1;
  END IF;
  ...
  -- Clamp sub_slot to valid range
  IF p_sub_slot > v_max_sub_slots THEN
    p_sub_slot := 1;
  END IF;
...
-- 3. Recreate validate_and_update_appointment with sub_slot validation
DROP FUNCTION IF EXISTS public.validate_and_update_appointment(uuid,uuid,date,time,integer,text,text,text);
...
CREATE OR REPLACE FUNCTION public.validate_and_update_appointment(
  ...
  p_sub_slot integer DEFAULT NULL,
...
BEGIN
  ...
  -- ── Defensive sub_slot validation ──
  IF p_sub_slot IS NOT NULL AND p_sub_slot < 1 THEN
    p_sub_slot := 1;
  END IF;
  ...
  v_treatment_name := CASE v_new_treatment_key
    WHEN 'fkt' THEN 'FKT' WHEN 'atm' THEN 'ATM'
    WHEN 'drenaje' THEN 'Drenaje linfática' WHEN 'drenaje_ultra' THEN 'Drenaje + Ultrasonido'
    WHEN 'masaje' THEN 'Masaje' WHEN 'vestibular' THEN 'Vestibular'
