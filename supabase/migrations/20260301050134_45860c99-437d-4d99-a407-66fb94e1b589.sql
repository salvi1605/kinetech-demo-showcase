...
-- ============================================================
-- Blindar validate_and_create_appointment con advisory lock
-- para prevenir race conditions de concurrencia
-- ============================================================
...
DECLARE
  ...
  v_lock_key BIGINT;
BEGIN
  -- ▶ ADVISORY LOCK: serializa operaciones por (practitioner + date + start_time)
  -- Usa hashtext para generar un bigint determinístico
  v_lock_key := hashtext(p_practitioner_id::text || p_date::text || p_start_time::text);
...
-- ============================================================
-- Blindar validate_and_update_appointment con advisory lock
-- ============================================================
...
BEGIN
  ...
  -- ▶ ADVISORY LOCK: serializa por (practitioner + date + start_time) destino
  v_lock_key := hashtext(v_new_practitioner_id::text || v_new_date::text || v_new_start_time::text);
  PERFORM pg_advisory_xact_lock(v_lock_key);

  -- Si cambió de slot, también lockear el slot origen para evitar doble-move
  IF v_date_or_time_changed THEN
    DECLARE
      v_old_lock_key BIGINT;
    BEGIN
      v_old_lock_key := hashtext(v_current.practitioner_id::text || v_current.date::text || v_current.start_time::text);
      IF v_old_lock_key <> v_lock_key THEN
