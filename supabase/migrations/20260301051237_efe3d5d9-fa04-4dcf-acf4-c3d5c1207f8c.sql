...
-- Reemplazar advisory locks en validate_and_create_appointment y validate_and_update_appointment
-- Esquema: pg_advisory_xact_lock(key1 int, key2 int) con orden consistente
...
DECLARE
  ...
  v_lock_key2 INT;
BEGIN
  -- ▶ ADVISORY LOCK (key1, key2): serializa por (practitioner + clinic|date|start_time)
...
-- ── validate_and_update_appointment con (key1, key2) + orden anti-deadlock ──
...
BEGIN
  ...
  -- ▶ ADVISORY LOCKS (key1 int, key2 int) con orden consistente anti-deadlock
  ...
  IF v_date_or_time_changed THEN
    ...
    ELSIF (v_src_key1 < v_dest_key1) OR (v_src_key1 = v_dest_key1 AND v_src_key2 < v_dest_key2) THEN
      -- SRC primero, luego DEST
      PERFORM pg_advisory_xact_lock(v_src_key1, v_src_key2);
      PERFORM pg_advisory_xact_lock(v_dest_key1, v_dest_key2);
    ELSE
      -- DEST primero, luego SRC
      PERFORM pg_advisory_xact_lock(v_dest_key1, v_dest_key2);
      PERFORM pg_advisory_xact_lock(v_src_key1, v_src_key2);
    END IF;
  ELSE
    -- Sin cambio de slot, solo lockear destino
    PERFORM pg_advisory_xact_lock(v_dest_key1, v_dest_key2);
  END IF;
