...
BEGIN
  ...
  -- ── 1.5 PRACTITIONER BLOCK CHECK (NEW) ──
  -- Reject if practitioner has a practitioner_block exception covering this date/time.
  SELECT EXISTS(
    ...
    WHERE clinic_id = p_clinic_id
      ...
      AND (
        ...
        (from_time IS NULL OR to_time IS NULL)
        -- Or partial block covering this start_time
        OR (p_start_time >= from_time AND p_start_time < to_time)
      )
  ), (
    SELECT reason FROM schedule_exceptions
    WHERE clinic_id = p_clinic_id
      AND practitioner_id = p_practitioner_id
      AND date = p_date
      AND type = 'practitioner_block'
