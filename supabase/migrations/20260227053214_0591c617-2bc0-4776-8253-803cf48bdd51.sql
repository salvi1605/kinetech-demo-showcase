...
-- ═══════════════════════════════════════════════════════════════
-- 1. Tabla clinical_note_versions
-- ═══════════════════════════════════════════════════════════════
CREATE TABLE public.clinical_note_versions (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  note_id UUID NOT NULL REFERENCES public.patient_clinical_notes(id) ON DELETE CASCADE,
...
CREATE INDEX idx_clinical_note_versions_note_date 
  ON public.clinical_note_versions (note_id, edited_at DESC);

ALTER TABLE public.clinical_note_versions ENABLE ROW LEVEL SECURITY;

-- Admin full access (via join to note -> patient -> clinic)
CREATE POLICY "note_versions_admin_full_access"
  ON public.clinical_note_versions FOR ALL
  USING (
    EXISTS (
      SELECT 1 FROM patient_clinical_notes n
      WHERE n.id = clinical_note_versions.note_id
        AND is_admin_clinic(n.clinic_id)
    )
  );

-- Health pro SELECT only (assigned patients)
CREATE POLICY "note_versions_pro_view_assigned"
  ON public.clinical_note_versions FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM patient_clinical_notes n
      JOIN appointments a ON a.patient_id = n.patient_id
      WHERE n.id = clinical_note_versions.note_id
        AND a.practitioner_id = current_practitioner_id()
    )
  );

-- Health pro INSERT (can create versions for their own edits)
CREATE POLICY "note_versions_pro_insert"
...
-- No receptionist policy = zero access

-- ═══════════════════════════════════════════════════════════════
-- 2. Modify validate_and_create_appointment: auto-create stub
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_block_reason IS NOT NULL THEN
    RETURN jsonb_build_object(
      'success', false, 'error_code', 'BLOCKED',
  ...
  IF NOT v_is_within_slot THEN
    ...
    IF v_has_any_availability THEN
      RETURN jsonb_build_object(
        'success', false, 'error_code', 'OUT_OF_HOURS',
  ...
  WHERE a.practitioner_id = p_practitioner_id AND a.date = p_date
    ...
    AND (v_is_candidate_exclusive OR LOWER(COALESCE(tt.name, '')) IN ('drenaje linfático', 'masaje'))
  LIMIT 1;

  IF v_existing_treatment IS NOT NULL OR (v_is_candidate_exclusive AND EXISTS(
    SELECT 1 FROM appointments
    WHERE practitioner_id = p_practitioner_id AND date = p_date AND start_time = p_start_time
      AND clinic_id = p_clinic_id AND status <> 'cancelled'
  )) THEN
    IF v_is_candidate_exclusive THEN
      IF EXISTS(
        ...
        RETURN jsonb_build_object(
          'success', false, 'error_code', 'EXCLUSIVE_CONFLICT',
          'error_message', 'Conflicto: el tratamiento ' || v_treatment_name || ' es exclusivo y ya hay citas en este horario'
        );
      END IF;
    ELSE
      IF EXISTS(
        SELECT 1 FROM appointments a LEFT JOIN treatment_types tt ON a.treatment_type_id = tt.id
        WHERE a.practitioner_id = p_practitioner_id AND a.date = p_date AND a.start_time = p_start_time
          AND a.clinic_id = p_clinic_id AND a.status <> 'cancelled'
...
-- ═══════════════════════════════════════════════════════════════
-- 3. Modify validate_and_update_appointment: move/cancel logic
-- ═══════════════════════════════════════════════════════════════
...
BEGIN
  ...
  IF v_date_or_time_changed THEN
    ...
    IF v_free_slot IS NULL THEN
      RETURN jsonb_build_object(
        'success', false, 'error_code', 'SLOT_FULL',
  ...
  -- ── 7. MOVE evolution if date/time/practitioner changed ──
  IF v_date_or_time_changed THEN
    UPDATE patient_clinical_notes
    SET note_date = v_new_date,
  ...
  -- ── 8. HANDLE CANCELLATION: delete empty / annotate non-empty ──
