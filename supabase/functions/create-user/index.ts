...
serve(async (req) => {
  ...
  try {
    ...
    const isSuperAdmin = userRoles?.some(ur => ur.role_id === 'super_admin')
    const isAdmin = isSuperAdmin || userRoles?.some(ur => ur.role_id === 'admin_clinic' || ur.role_id === 'tenant_owner')
    
    if (!isAdmin) {
      return new Response(
        JSON.stringify({ error: 'Only admins, tenant owners, or super admins can create users' }),
